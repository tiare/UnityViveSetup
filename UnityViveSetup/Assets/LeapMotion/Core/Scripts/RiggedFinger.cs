...
*/
  public class RiggedFinger : FingerModel {
    ...
    public Quaternion Reorientation() {
            //return Quaternion.Inverse(Quaternion.LookRotation(modelFingerPointing, -modelPalmFacing)) * Quaternion.AngleAxis(GetHandShiftAngle(), GetHandShiftAxis());//Vector3.Cross(modelFingerPointing, -modelPalmFacing)); //added by Tiare;                
      ...
      for (int i = 0; i < bones.Length; ++i) {
        if (bones[i] != null) {
          bones[i].rotation = GetBoneRotation(i) * Reorientation();
                    bones[i].Rotate(GetHandShiftAxis(), GetHandShiftAngle(), UnityEngine.Space.World);
          if (deformPosition) {
