...
namespace Leap.Unity
{
    ...
    */
    ...
    public abstract class FingerModel : MonoBehaviour
    {
        ...
        private Vector3 handPositionOffset = Vector3.zero, shiftRotationAxis = Vector3.zero; //added by Tiare
        private float handShiftAngle = 0; //added by Tiare
        ...
        * Added by Tiare
        */
        public void SetHandPositionAndRotationOffset(Vector3 offset, Vector3 axis, float angleOffset)
        {
            handPositionOffset = offset;
            handShiftAngle = angleOffset;
        ...
        */
        ...
        public Vector3 GetTipPosition()
        {
            if (finger_ != null)
            {
                ...
                //return local_tip;
                return local_tip + handPositionOffset; //edit by Tiare
            }
            if (bones[NUM_BONES - 1] && joints[NUM_JOINTS - 2])
            {
                //return 2f*bones [NUM_BONES - 1].position - joints [NUM_JOINTS - 2].position;
                return 2f * bones[NUM_BONES - 1].position - joints[NUM_JOINTS - 2].position + handPositionOffset; //edit by Tiare
            }
            return Vector3.zero;
        }
        ...
        public Vector3 GetJointPosition(int joint)
        {
            ...
            if (finger_ != null)
            {
                ...
                return local_position + handPositionOffset; //edit by Tiare
                //return local_position;
            }
            if (joints[joint])
            {
                //return joints[joint].position;
                return joints[joint].position + handPositionOffset; //edit by Tiare
            }
            return Vector3.zero;
        }
        ...
        public Vector3 GetBoneCenter(int bone_type)
        {
            if (finger_ != null)
            {
                ...
                return bone.Center.ToVector3() + handPositionOffset; //edit by Tiare
                //return bone.Center.ToVector3();
            }
            if (bones[bone_type])
            {
                return bones[bone_type].position + handPositionOffset; //edit by Tiare
