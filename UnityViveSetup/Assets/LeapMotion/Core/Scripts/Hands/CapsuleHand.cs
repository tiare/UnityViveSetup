...
namespace Leap.Unity
{
    ...
    public class CapsuleHand : IHandModel
    {
        ...
        private Vector3 handPositionOffset;//, elbowPosition; //Added by Tiare
        //private float armShiftAngle; //Added by Tiare
        ...
        public override void UpdateHand()
        {
            ...
            foreach (var finger in _hand.Fingers)
            {
                for (int j = 0; j < 4; j++)
                {
                    ...
                    //position.x += handOffset; //Edit by Tiare
                    position += handPositionOffset; //Edit by Tiare
                    _spherePositions[key] = position;

                    //Debug.Log("Finger " + finger.Type + " at joint " + j + ": " + position);
            ...
            Vector3 palmPosition = GetPosition(); //Edit by Tiare
            drawSphere(palmPosition, PALM_RADIUS);

            //Vector3 wristPos = _hand.PalmPosition.ToVector3();
            Vector3 wristPos = palmPosition; //Edit by Tiare
            drawSphere(wristPos);

            Vector3 thumbBaseToPalm = _spherePositions[THUMB_BASE_INDEX] - palmPosition; //Edit by Tiare//_hand.PalmPosition.ToVector3();
            Vector3 mockThumbJointPos = palmPosition + Vector3.Reflect(thumbBaseToPalm, _hand.Basis.xBasis.ToVector3()); //Edit by Tiare _hand.PalmPosition.ToVector3() + Vector3.Reflect(thumbBaseToPalm, _hand.Basis.xBasis.ToVector3());
            ...
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    //Debug.Log("Draw finger " + i);
                    drawCylinder(posA, posB);
                }
            }

            //Debug.Log("Draw knuckles");
            ...
            //Debug.Log("Draw rest of hand");
            //Draw the rest of the hand
            drawCylinder(mockThumbJointPos, THUMB_BASE_INDEX); //bottom cylinder closing the hand at the arm
            drawCylinder(mockThumbJointPos, PINKY_BASE_INDEX); //cylinder at outer edge of hand
        }
        ...
        public override void UpdateHandOffset(Vector3 positionOffset, Vector3 armLookAtPos, Vector3 shiftAxis, float shiftAngle) //added by Tiare
        {
            //Debug.Log("CapsuleHand");
            handPositionOffset = positionOffset;
            //not using  elbowPosition = armLookAtPos;
            //not using  armShiftAngle = shiftAngle;
            //not using Vector3 shiftAxis
        }

        public override Vector3 GetPosition() //added by Tiare
        {
            Vector3 new_pos = _hand.PalmPosition.ToVector3(); //Edit by Tiare
            new_pos += handPositionOffset; //Edit by Tiare
            return new_pos;
        }

        public override void FreezeFingers(bool rightHand)  //added by Tiare
        {
            //not doing anything in this implementation
        }
    }
}
