...
public class SteamVR_RenderModel : MonoBehaviour
{
	...
	private void StripMesh(UnityEngine.GameObject go)
	{
	...
	private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				t = new UnityEngine.GameObject(componentName.ToString()).transform;
				t.parent = transform;
				t.gameObject.layer = gameObject.layer;

				// Also create a child 'attach' object for attaching things.
				var attach = new UnityEngine.GameObject(k_localTransformName).transform;
				attach.parent = t;
