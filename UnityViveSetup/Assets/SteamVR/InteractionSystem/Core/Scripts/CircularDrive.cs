...
namespace Valve.VR.InteractionSystem
{
	...
	public class CircularDrive : MonoBehaviour
	{
		...
		private void DrawDebugPath( Transform xForm, Vector3 toTransformProjected )
		{
			if ( dbgObjectCount == 0 )
			{
                dbgObjectsParent = new UnityEngine.GameObject( "Circular Drive Debug" );
                dbgHandObjects = new UnityEngine.GameObject[dbgPathLimit];
                dbgProjObjects = new UnityEngine.GameObject[dbgPathLimit];
				dbgObjectCount = dbgPathLimit;
				dbgObjectIndex = 0;
			}

            //Actual path
            UnityEngine.GameObject gSphere = null;
			...
			{
				gSphere = UnityEngine.GameObject.CreatePrimitive(PrimitiveType.Sphere );
