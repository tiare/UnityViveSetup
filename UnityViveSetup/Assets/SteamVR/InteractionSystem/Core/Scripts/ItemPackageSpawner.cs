...
namespace Valve.VR.InteractionSystem
{
	...
	public class ItemPackageSpawner : MonoBehaviour
	{
		...
		private void CreatePreviewObject()
		{
			...
			if ( useItemPackagePreview )
			{
				...
				if ( useFadedPreview == false ) // if we don't have a spawned item out there, use the regular preview
				{
					...
					{
                        previewObject = Instantiate(itemPackage.previewPrefab, transform.position, Quaternion.identity) as UnityEngine.GameObject;
				...
				else // there's a spawned item out there. Use the faded preview
				{
					...
					{
                        previewObject = Instantiate(itemPackage.fadedPreviewPrefab, transform.position, Quaternion.identity) as UnityEngine.GameObject;
						previewObject.transform.parent = transform;
						previewObject.transform.localRotation = Quaternion.identity;
					}
				}
		...
		{
            UnityEngine.GameObject currentAttachedObject = hand.currentAttachedObject;
		...
		private void RemoveMatchingItemsFromHandStack( ItemPackage package, Hand hand )
		{
			for ( int i = 0; i < hand.AttachedObjects.Count; i++ )
			{
				...
				if ( packageReference != null )
				{
					...
					{
                        UnityEngine.GameObject detachedItem = hand.AttachedObjects[i].attachedObject;
		...
		private void SpawnAndAttachObject( Hand hand )
		{
            ...
            spawnedItem = UnityEngine.GameObject.Instantiate(itemPackage.itemPrefab );
			...
			{
                UnityEngine.GameObject otherHandObjectToAttach = UnityEngine.GameObject.Instantiate(itemPackage.otherHandItemPrefab );
